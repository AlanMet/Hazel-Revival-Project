...
class App:
    ...
    def print_connect_success(self, summary: ConnectSummary, *, already: bool = False) -> None:
        ...
        lines = [
            ...
            "",
            "[dim]GATT snapshot saved to reverse-engineering/research/gatt-map.md[/dim]",
    ...
    def _session_log_hint(self) -> str:
        ...
        if path is None:
            return "[dim]Session log: reverse-engineering/research/captures/session-*.log[/dim]"
    ...
    async def menu_gatt_explorer(self) -> None:
        ...
        if input("\nRead all readable characteristics now? [Y/n]: ").strip().lower() != "n":
            ...
            console.print(f"[dim]Saved to {path}[/dim]")
        console.print("\n[dim]Copy service/write/notify UUIDs into reverse-engineering/tools/config/gatt_map.yaml[/dim]")
    ...
    def menu_session_log(self) -> None:
        ...
        for line in lines:
            if line.startswith("#"):
                ...
                console.print(line)
        if input("\nExport JSON snapshot to reverse-engineering/research/captures/? [y/N]: ").strip().lower() == "y":
