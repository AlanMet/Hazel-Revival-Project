"""Load dev configuration from reverse-engineering/tools/config/gatt_map.yaml."""
...
def project_root() -> Path:
    """Resolve reverse-engineering directory (contains tools/config/gatt_map.yaml)."""
    here = Path(__file__).resolve().parent
    candidate = here.parent.parent
    if (candidate / "tools" / "config" / "gatt_map.yaml").exists():
        return candidate
    cwd = Path.cwd()
    re_from_cwd = cwd / "reverse-engineering"
    if (re_from_cwd / "tools" / "config" / "gatt_map.yaml").exists():
        return re_from_cwd
