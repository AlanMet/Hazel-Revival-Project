...
All writes go to **`52401524`** as **two ATT writes**: header (includes 24-bit payload length), then payload. Builders: `reverse-engineering/src/zephyr_re_dev/protocol/hazel_write.py`, templates: `reverse-engineering/tools/config/gatt_map.yaml`.

Regenerate reference hex:

```bash
python reverse-engineering/tools/scripts/hazel_packet_dump.py --all
```
...
Order: wave → spectrum → breathing → off → static → brightness → internal → fans. Regenerate with `python reverse-engineering/tools/scripts/hazel_packet_dump.py --all`.
...
5. `python reverse-engineering/tools/scripts/hazel_packet_dump.py --all` — reference write hex
