# GATT Map — Razer Zephyr

Human-readable notes and auto-appended snapshots from connects.

## How to use

1. Connect to the mask via the tool
2. Each connect appends a **Snapshot** section below with full service/characteristic list
3. Copy the vendor service UUID, write char, and notify char into `reverse-engineering/tools/config/gatt_map.yaml`
...
| Field | UUID | Notes |
|-------|------|-------|
| Service | `52401523-F97C-7F90-0E7F-6C6F4E36DB1C` | Same as Razer mouse BLE (OpenSnek) |
...
| Doc | Contents |
|-----|----------|
| [protocol.md](protocol.md) | Verification matrix, read/write keys, reference hex |
| [device-state.md](device-state.md) | Sync fields, write table, session example |
...
```bash
python scripts/hazel_packet_dump.py --all   # reference write pairs
```

Builders: `reverse-engineering/src/zephyr_re_dev/protocol/hazel_write.py` · Templates: `reverse-engineering/tools/config/gatt_map.yaml`
