"""YAML-driven protocol backend — send commands from tools/config/gatt_map.yaml."""
...
class YamlBackend(ProtocolBackend):
    ...
    async def on_connect(self, client: BleClient) -> None:
        ...
        if self.configured:
            ...
            client.capture.log(
                "info",
                message="[yaml] write_char_uuid / notify_char_uuid not set — edit tools/config/gatt_map.yaml",
            )
    ...
    async def _run_command(self, spec: CommandSpec | None, **fmt: Any) -> CommandResult:
        ...
        if not self.configured:
            return CommandResult(
                ...
                "Set write_char_uuid and notify_char_uuid in tools/config/gatt_map.yaml",
            )
